...
INSERT INTO `access_token` VALUES (2,'343434',1,65),(3,'$2y$10$l0F0yJuA2OOv8YYfG0Yhh.z5nacTOnmoXl1M/tPLv6g4s7cklLObC',-1,69);
...
INSERT INTO `verify_status` VALUES (60,58,'916396',1490281712,0,3),(61,59,'646461',1490281892,0,1),(62,60,'165365',1490281955,0,1),(63,61,'797984',1350285136,0,1),(64,62,'363004',1390285315,0,1),(65,63,'771786',1490285468,0,1),(66,64,'714450',1490285911,0,1),(67,65,'228572',1490286213,0,2),(68,66,'513847',1490618112,0,3),(69,67,'598001',1490619219,1,3),(70,68,'841274',1490719667,0,1),(71,69,'507641',1490722884,0,3);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_token`(
	in _user_id int,
    in _token VARCHAR(128),
    in _expired double)
BEGIN
	declare _id int;
    set @_id := 0;
    
    select @_id := `id` from access_token
    where (user_id = _user_id);
        
    if (@_id = 0)
    then
		begin
			insert into access_token(user_id, access_token, expired)
            values (_user_id, _token, _expired);
            select LAST_INSERT_ID();
		end;
	else
        begin
			update access_token 
            set access_token = _token, expired = _expired
            where (id = @_id);
            select @_id;
        end;
	end if;
...
-- Dump completed on 2017-03-26 17:02:34
